...
type ExecutiveStatusPayload = ExecutiveSchedulerStatus & {
  processing?: ExecutiveProcessingStatusView;
};
...
function StatusBadge({
  ...
  status:
    | ExecutiveProcessingStatusView["status"]
    | ExecutiveSchedulerStatus["statusLabel"];
}) {
...
/**
 * Executive scheduler panel — mirrors `lateral-scheduler-panel.tsx`
 * structurally (Phase E8). Two differences from Lateral's panel:
 * - No "Edit Setup" button/wizard — Executive has no XLSM master workbook
 *   to configure, so there's nothing that heavy to edit. Instead, an inline
 *   "Edit Schedule" affordance below covers frequency/time/timezone, since
 *   that's the only thing Lateral's wizard covered that Executive still
 *   needs a UI for.
 * - Talks to `/api/dataset/executive/scheduler` and
 *   `/api/dataset/executive/sync-history` — fully independent endpoints,
 *   backed by `executive-scheduler.ts` / `executive_sync_history` (own
 *   table), never touching any Lateral route or table.
 */
export function ExecutiveSchedulerPanel() {
  const [status, setStatus] = React.useState<ExecutiveStatusPayload | null>(
    null
  );
  ...
  const [history, setHistory] = React.useState<ExecutiveSyncHistoryEntry[]>(
    []
  );
  ...
  const refresh = React.useCallback(async () => {
    const response = await fetch("/api/dataset/executive/scheduler");
    const payload = (await response.json().catch(() => null)) as
      | ExecutiveStatusPayload
      | { error?: string }
      | null;
    if (!response.ok) {
      throw new Error(
        (payload as { error?: string } | null)?.error ??
          "Failed to load Executive status."
      );
    }
    setStatus(payload as ExecutiveStatusPayload);
  }, []);
  ...
  const loadHistory = React.useCallback(async () => {
    ...
    try {
      const response = await apiFetch(
        "/api/dataset/executive/sync-history?limit=100"
      );
      const payload = (await response.json().catch(() => null)) as {
  ...
  async function runAction(
    action: "run_now" | "pause" | "resume" | "reload" | "update",
    ...
    try {
      ...
      if (action === "run_now") {
        setInfo(payload?.outcome?.message ?? "Executive job finished.");
        if (showHistory) void loadHistory();
      } else if (action === "pause") {
        setInfo("Executive schedule paused.");
      } else if (action === "resume") {
        setInfo("Executive schedule resumed.");
      }
    } catch (err) {
  ...
  if (!status) {
    return (
      <div className="rounded-xl border border-border/70 bg-muted/20 px-3 py-3 text-sm text-muted-foreground">
        {error ?? "Loading Executive processing status…"}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <p className="text-base font-semibold text-foreground">Executive</p>
          <p className="text-xs text-muted-foreground">
            Processing status for Executive Dataset automation.
          </p>
        </div>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <label className="space-y-1 text-xs text-muted-foreground">
              ...
              <select
                ...
                onChange={(e) =>
                  setScheduleForm((f) =>
                    f ? { ...f, frequency: e.target.value } : f
                  )
                }
              >
            ...
            <label className="space-y-1 text-xs text-muted-foreground">
              ...
              <input
                type="time"
                className="block w-full rounded-lg border border-border/70 bg-background px-2 py-1.5 text-sm text-foreground"
                value={scheduleForm.syncTime}
                onChange={(e) =>
                  setScheduleForm((f) =>
                    f ? { ...f, syncTime: e.target.value } : f
                  )
                }
              />
            </label>
            ...
            <table className="min-w-[64rem] w-full text-left text-xs">
              ...
              <tbody>
                {historyBusy && history.length === 0 ? (
                  ...
                  <tr>
                    <td colSpan={11} className="px-3 py-4 text-muted-foreground">
                      No Executive sync runs recorded yet.
                    </td>
                  </tr>
                ) : (
                  history.map((entry) => (
