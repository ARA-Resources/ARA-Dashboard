/**
 * Stage 30A: Lateral processing preview data reader.
 * Matches Next src/services/lateral-processing/data-reader.ts.
 */
...
import type { LateralDataProcessingSetup } from "../../types/lateral-processing-setup.js";
import type {
  ColumnMapping,
  ColumnMappingFailure,
  DataReadResult,
  SheetReadResult,
} from "../../types/lateral-processing-preview.js";
import { buildColumnMappingByHeaderName } from "./lateral-column-mapping.js";
import {
  EXPECTED_NEW_SHEET_HEADERS,
  headersMatchIgnoringCase,
} from "./lateral-new-sheet-structure.js";
import { formatProcessingDateDDMMYYYY } from "./lateral-new-sheet-refresh.js";
...
async function downloadToTemp(fileId: string, nameHint: string): Promise<string> {
  ...
  const tempPath = path.join(
    os.tmpdir(),
    `lateral-read-${Date.now()}-${safeName}`
  );
